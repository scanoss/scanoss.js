...
export class LicenseDataProvider implements DataProvider {
    ...
    this.componentList.forEach((component) => {
      component.licenses.forEach((license) => {
        ...
        if (!licenseExist) {
          ...
          this.licenseStorage[license.name] = this.insertComponentIntoLicense(
            this.licenseStorage[license.name],
            newLicenseComponent
          );
        }
      });
        ...
        dependency.licensesList.forEach((license) => {
          ...
          if (license.spdxId !== '') {
            license.spdxId.split(/;|\//g).forEach((license_name) => {
              ...
              if (!licenseExist) {
                ...
                this.licenseStorage[license_name] =
                  this.insertComponentIntoLicense(
                    this.licenseStorage[license_name],
                    newLicenseComponent
                  );
              }
            ...
            if (!licenseExist) {
              ...
              this.licenseStorage['unknown'] = this.insertComponentIntoLicense(
                this.licenseStorage['unknown'],
                newLicenseComponent
              );
            }
          }
  ...
  private insertComponentIntoLicense(
    ...
    const componentIndex = license.components.findIndex(
      (c) => c.purl === newComponent.purl
    );
    if (componentIndex >= 0) {
      //if newComponent exist in license
      const versionExist = !!license.components[componentIndex].versions.find(
        (version) => version === newComponent.versions[0]
      );
      if (!versionExist) {
        license.components[componentIndex].versions.push(
          newComponent.versions[0]
        );
        license.value++;
      }
    } else {
