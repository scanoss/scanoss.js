...
export abstract class BaseScanner extends EventEmitter  {

  private readonly SCAN_FOLDER_NAME = 'scanner';

  private scannerCfg: ScannerCfg;

  protected workDirectory: string;
  ...
  public setScannerCfg(scannerCfg: ScannerCfg){
    this.scannerCfg = scannerCfg;
  }

  protected getScanFolderId(){
    return `${this.SCAN_FOLDER_NAME}-${this.getScannerId()}`;
  }


  public abstract scan(scanner: ScannerVisitor);
  ...
  protected async startScan(scannerInput: Array<ScannerInput>):Promise<ScannerResponse> {
    this.init();
    ...
    if (!this.isValidInput(scannerInput)) {
      this.finishScan();
      return { resultPath : await this.finishPromise };
    }
 ...
 /* public async scanContents(contentScannerInput: ContentScannerInput):Promise<ScannerComponent | null> {
  ...
  protected reportLog(txt, level = 'info') {
