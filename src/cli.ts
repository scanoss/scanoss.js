...
async function main() {
  ...
  scan.addOption(
    new Option("-x, --extract", "Extract compressed files before launch scan in folder <<zip_name>>-unzipped")
  );
  ...
  scan.addOption(
    new Option("-M, --timeout <timeout>", "Timeout (in seconds) for API communication (optional -default 120)")
  );
  ...
  scan.addOption(
    new Option("    --apiurl <apiurl>", "SCANOSS API URL (optional - default: https://osskb.org/api/scan/direct)")
  );
  scan.addOption(
    new Option("    --api2url <api2url>", "SCANOSS gRPC API 2.0 URL (optional - default: scanoss.com:443)")
  );
  scan.addOption(
    new Option("-k, --key <key>", "SCANOSS API Key token (optional - not required for default OSSKB URL)")
  );
  ...
  scan.addOption(
    new Option("    --pac <pac>", "Proxy auto configuration (optional). Specify a file, http url or ftp url")
  );
  ...
  dependencies.addOption(
    new Option("-a, --grpc-host <host>", "SCANOSS GRPC HOST (optional - default: scanoss.com:443)")
  );
  ...
  fingerprint.addOption(
    new Option("-p, --block-size <size>", "Maximum size in Kb for each fingerprint block (optional - default 64Kb)")
  );
